    ...
    function utilityService($http,profileService) {
      var profile = this;
        ...
        profile.loadOrganisationUnits = function(){
            ...
            return $http.get(profile.baseDHIS+'api/organisationUnits.json?filter=level:eq:1&paging=false&fields=id,name,children[id,name,children[id,name,children[ null]]]').then(handleSuccess, handleError('Error creating user'));
        }
        ...
        profile.login = function(username,password){
           ...
           return $http.get(profile.baseDHIS+'/api/me.json').then(function(user){
